...
function vueDocWayFormat(text){
	var propertyFormat = '';
	var textArr = text.split('\n');
	console.log(textArr);
	for(var i = 0;i < textArr.length;i += vueDocWayFormatSepNum){
		if(i + 3 < textArr.length){
			var prpoTypeDefaultValue;
			var prpoText = textArr[i];
			let prpoType = textArr[i + 1];
			//let prpoDefaultValue = textArr[i + 2];
			let prpoComment = textArr[i + 3];
			
			prpoText = handleKeyConvert(prpoText);
			
			if(prpoType == 'string'){
				prpoTypeDefaultValue = '\'\'';
			}else if(prpoType == 'number' || prpoType == 'int32' || prpoType == 'int64'){
				prpoTypeDefaultValue = '0';
			}else if(prpoType == 'boolean'){
				prpoTypeDefaultValue = 'false';
			}else if(prpoType.startsWith('array')){
				prpoTypeDefaultValue = '[]';
			}else{
				prpoTypeDefaultValue = 'null';
			}
			if(localStorage.getItem('addComment') == 'true'){
				propertyFormat += '//' + prpoComment + '\n';
			}
			propertyFormat += prpoText + ' : ' + prpoTypeDefaultValue + ',\n';
		}
	}
	if(propertyFormat.length > 2){
		propertyFormat = propertyFormat.substr(0, propertyFormat.length - 2);
	}
	return propertyFormat;
}
		...
		convertAction(){
			...
			try {
			   ...
			   this.outputValue = resStr;
				...
				if(vm2.language == 'Vue'){
					this.outputValue = vueDocWayFormat(this.inputValue);
